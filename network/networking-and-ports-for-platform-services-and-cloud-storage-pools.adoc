...
Cloud Storage Pools erfordern ebenfalls Zugriff von den Storage Nodes auf die Endpunkte des verwendeten externen Dienstes, wie beispielsweise Amazon S3 Glacier oder Microsoft Azure Blob Storage. Weitere Informationen sind unter link:../ilm/what-cloud-storage-pool-is.html["Was ist ein Cloud-Storage-Pool"] zu finden.
