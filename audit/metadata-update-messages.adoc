...
---
= Revisionsprotokoll-Meldungen für Aktualisierungen der S3-Objektmetadaten in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Der S3-Client stellt eine Anfrage (SUPD), um die angegebenen Metadaten (`x-amz-meta-*` für das S3-Objekt (S3KY) zu aktualisieren.

In diesem Beispiel sind die Anfrageheader im Feld HTRH enthalten, da es als Audit-Protokollheader konfiguriert wurde (*Konfiguration* > *Überwachung* > *Audit- und Syslog-Server*). Siehe link:../monitor/configure-log-management.html["Konfigurieren Sie die Protokollverwaltung und den externen Syslog-Server"].
