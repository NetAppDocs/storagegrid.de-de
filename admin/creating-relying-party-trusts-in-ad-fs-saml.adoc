...
---
= Erstellen von Vertrauensstellungen für vertrauende Parteien in AD FS für SAML in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie müssen Active Directory Federation Services (AD FS) verwenden, um für jeden Admin Node in Ihrem System eine Vertrauensstellung für SAML zu erstellen. Vertrauensstellungen können mithilfe von PowerShell-Befehlen, durch Importieren von SAML-Metadaten aus StorageGRID oder durch manuelle Dateneingabe erstellt werden.

.Bevor Sie beginnen
* Sie haben Single Sign-On für StorageGRID konfiguriert und als SSO-Typ *AD FS* ausgewählt.
* Sie haben link:../admin/configure-sso-saml.html["Sandbox-Modus aufgerufen"] im Grid Manager.
...
. Wenn Sie fertig sind, kehren Sie zu StorageGRID zurück und link:../admin/configure-sso-saml.html#test-sso["Testen Sie alle Vertrauensstellungen der vertrauenden Parteien"] um zu bestätigen, dass sie korrekt konfiguriert sind.
