...
. Wenn das Banner *FabricPool and S3 setup wizard* im Dashboard angezeigt wird, den Link im Banner auswählen. Wenn das Banner nicht mehr angezeigt wird, das Hilfesymbol image:icon_help_menu_bar.png["Hilfesymbol"] in der Kopfzeile des Grid Manager auswählen und *FabricPool and S3 setup wizard* auswählen.
...
*Hinweis:* Von anderen Grid-Diensten verwendete Ports sind nicht zulässig. Sehen link:../network/internal-grid-node-communications.html#storagegrid-internal-ports["Interne StorageGRID-Ports"] .
...
. Definieren Sie den Root-Zugriff für das Mandantenkonto, je nachdem, ob Ihr StorageGRID -Systemlink:using-identity-federation.html["Identitätsföderation"] ,link:how-sso-works.html["Single Sign On (SSO)"] oder beides.
+
[cols="1a,2a"]
|===
| Option | Tun Sie das 
...
** Wählen Sie das Kopiersymbol (image:icon_tenant_copy_url.png["Symbol kopieren"]), um die Zugriffsschlüssel-ID und den geheimen Zugriffsschlüssel in die Zwischenablage zu kopieren.
