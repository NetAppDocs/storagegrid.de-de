...
[role="lead"]
Wenn Sie Azure als SSO-Anbieter haben link:../admin/how-sso-works.html["Konfiguration und Aktivierung von Single Sign On (SSO)"] und verwenden, können Sie zwei Beispielskripte verwenden, um ein Authentifizierungstoken zu erhalten, das für die Grid-Management-API oder die Mandantenmanagement-API gültig ist.
