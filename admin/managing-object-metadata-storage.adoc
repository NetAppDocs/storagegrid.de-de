...
image:metadata_space_across_sites.png["Metadatenspeicherplatz Über Mehrere Standorte Hinweg"]
...
image:metadata_space_storage_node.png["Metadaten-Speicherplatz-Storage-Node"]
...
Die Standardeinstellung für eine neue StorageGRID 12.1 oder höhere Installation, bei der jeder Speicherknoten über 128 GB oder mehr RAM verfügt, beträgt 20.000 GB (20 TB).
...
+
image:storage_used_object_metadata_actual_reserved.png["Verwendeter Speicher - Objektmetadaten - Ist Reserviert"]



Im Screenshot beträgt der tatsächlich reservierte Wert 20 TB. Dieser Screenshot stammt von einem großen Storage Node in einer neuen StorageGRID 12.1 Installation. Volume 0 ist mindestens 20 TB groß, daher wird die systemweite Standardeinstellung für reservierten Metadatenspeicherplatz verwendet.



[[example-for-actual-reserved-metadata-space]]
=== Beispiel für tatsächlich reservierten Metadatenbereich

Angenommen, Sie installieren ein neues StorageGRID System mit Version 12.1 oder höher. Für dieses Beispiel wird angenommen, dass jeder Storage Node über mehr als 128 GB RAM verfügt und dass Volume 0 von Storage Node 1 (SN1) 6 TB groß ist. Basierend auf diesen Werten:

* Der systemweite *reservierte Speicherplatz* ist auf 20 TB festgelegt. (Dies ist der Standardwert für eine neue StorageGRID 12.1 oder höher Installation, wenn jeder Storage Node über mehr als 128 GB RAM verfügt.)
...
image:metadata_allowed_space_volume_0.png["Platz für Volume 0 durch Metadaten zulässig"]
...
&Lt;= 4 TB
 a| 
75 % des tatsächlich reservierten Speicherplatzes für Metadaten, bis zu maximal 1,32 TB
 a| 
75 % des tatsächlich reservierten Speicherplatzes für Metadaten, bis zu maximal 1,98 TB
...
> 4 TB
 a| 
(Tatsächlich reservierter Speicherplatz für Metadaten − 1 TB) × 75%, bis maximal 1,32 TB
 a| 
(Tatsächlich reservierter Speicherplatz für Metadaten − 1 TB) × 75%, bis maximal 7,92 TB
...
+
image:storage_used_object_metadata_allowed.png["Verwendeter Speicher - Objekt-Metadaten - Zulässig"]



Im Screenshot ist der Wert *Zulässig* mit 7,92 TB angegeben, was der Maximalwert für einen Storage Node ist, dessen tatsächlich reservierter Speicherplatz für Metadaten mehr als 4 TB beträgt.
...
* Der systemweite *reservierte Speicherplatz* ist auf 20 TB festgelegt. (Dies ist der Standardwert für eine neue StorageGRID 12.1 oder höher Installation, wenn jeder Storage Node über mehr als 128 GB RAM verfügt.)
* Der *tatsächlich reservierte* Speicherplatz für Metadaten für SN1 beträgt 6 TB. (Das gesamte Volume ist reserviert, da Volume 0 kleiner ist als die Einstellung *Reservierter Speicherplatz für Metadaten*.)
* Der *zulässige* Speicherplatz für Metadaten auf SN1 beträgt 3,75 TB, basierend auf der in der <<table-allowed-space-for-metadata,Tabelle für zulässigem Speicherplatz für Metadaten>> dargestellten Berechnung: (Tatsächlich reservierter Speicherplatz für Metadaten − 1 TB) × 75 %, bis zu einem Maximum von 7,92 TB.
...
image:metadata_space_three_storage_nodes.png["Metadaten Platz Drei Storage-Nodes"]
