...
summary: Cross-grid replication ist die automatische Replikation von Objekten zwischen ausgewählten S3-Buckets in zwei StorageGRID Systemen, die über eine Grid-Federation-Verbindung verbunden sind. Für die Cross-grid replication ist eine Account-Synchronisierung erforderlich. 
---
...
[role="lead"]
Die Cross-grid-Replikation ist die automatische Replikation von Objekten zwischen ausgewählten S3-Buckets in zwei StorageGRID Systemen, die in einer link:grid-federation-overview.html["Netzverbundverbindung"] verbunden sind. link:grid-federation-what-is-account-sync.html["Kontosynchronisierung"]Für die Cross-grid-Replikation ist eine entsprechende Konfiguration erforderlich.
...
image:grid_federation_cgr_workflow.png["Workflow für Grid-übergreifende Replizierung"]
...
Besitzt ein Mandantenkonto die Berechtigung *Grid-Federationsverbindung verwenden*, um eine oder mehrere link:grid-federation-overview.html["Netzverbundverbindungen"] zu nutzen, kann ein Mandantenbenutzer mit Root-Zugriffsberechtigung Buckets („Source“- und „Destination“-Buckets) in den entsprechenden Mandantenkonten auf jedem Grid erstellen. Diese Buckets:

* Können unterschiedliche Namen haben
* Kann verschiedene Regionen haben
* Versionierung muss aktiviert sein
* Eine der folgenden S3 Object Lock Konfigurationen muss vorhanden sein:
+
** Quelle: *deaktiviert*, Ziel: *deaktiviert*
** Quelle: *aktiviert*, Ziel: *aktiviert*
** Quelle: *deaktiviert*, Ziel: *aktiviert*




Nachdem Sie beide Buckets erstellt haben, kann die gridübergreifende Replikation für einen oder beide Buckets konfiguriert werden.
...
image:grid_federation_cross_grid_replication_one_direction.png["Abbildung zeigt die Verbindung mit dem Grid Federation in eine Richtung"]
...
image:grid_federation_cross_grid_replication.png["Bild zeigt Replikation in eine Richtung im Vergleich zur Replikation in beide Richtungen"]
...
image:grid_federation_cross_grid_replication_delete.png["Abbildung zeigt, wie der Replikate-Client auf beiden Rastern gelöscht wird"]
...
CAUTION: Sie können die S3 Object Lock für bestehende Buckets aktivieren, was später zu einer Fehlkonfiguration führen könnte.


NOTE: Wenn S3 Object Lock im Quell-Bucket aktiviert ist, sollte es auch im Ziel-Bucket aktiviert sein. Ist S3 Object Lock im Ziel-Bucket deaktiviert, tritt während der Replikation ein Fehler auf. Siehe link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["Grid-übergreifende Replizierungswarnungen und Fehler"].
