...
summary: Möglicherweise treten Warnmeldungen und Fehler im Zusammenhang mit Grid Federation-Verbindungen, Kontosynchronisierung und gridübergreifender Replizierung auf, die eine Fehlerbehebung erfordern. 
---
...
[role="lead"]
Möglicherweise treten Warnmeldungen und Fehler im Zusammenhang mit Grid Federation-Verbindungen, Kontosynchronisierung und gridübergreifender Replizierung auf, die eine Fehlerbehebung erfordern.
...
[[account-sync-errors]]
== Kontosynchronisierungsfehler



[[cant-sign-in-to-a-synced-tenant-account]]
=== Anmeldung bei einem synchronisierten Mandantenkonto nicht möglich

.Problem
Sie können sich nicht bei einem synchronisierten Mandantenkonto anmelden. Die Fehlermeldung auf der Anmeldeseite des Tenant Manager lautet: „Ihre Anmeldeinformationen für dieses Konto sind ungültig. Bitte versuchen Sie es erneut.“

.Details
Das für den lokalen Root-Benutzer des Mandanten und andere lokale Benutzer festgelegte Passwort wird automatisch synchronisiert. Es kann jedoch erforderlich sein, das Passwort für den lokalen Root-Benutzer des Mandanten im Ziel-Grid zu ändern, bevor eine Anmeldung am Mandantenkonto auf diesem Grid möglich ist.
...
=== Mandant ohne Synchronisierung erstellt

.Problem
Nach dem Erstellen eines neuen Mandanten mit der Berechtigung *Grid-Federationsverbindung verwenden* erscheint die Meldung „Mandant ohne Synchronisierung erstellt“.
...
. Das Mandantenkonto, das nicht synchronisiert werden konnte, ermitteln.
. Wählen Sie den Namen der Serviceeinheit aus, um die Detailseite anzuzeigen.
. *Synchronisierung wiederholen* auswählen. Wenn der Fehler behoben wurde, wird das Mandantenkonto nun mit dem anderen Grid synchronisiert.
...
image:grid_federation_last_error.png["Screenshot mit einer Meldung in der Spalte Letzter Fehler einer Grid Federation-Verbindung"]
...
. Alle empfohlenen Maßnahmen ausführen.
+
Wenn beispielsweise die Versionierung für den Ziel-Bucket bei der netzübergreifenden Replikation ausgesetzt wurde, sollte die Versionierung für diesen Bucket wieder aktiviert werden. Oder wenn S3 Object Lock für den Ziel-Bucket bei der netzübergreifenden Replikation deaktiviert wurde, sollte S3 Object Lock für diesen Bucket aktiviert werden.
