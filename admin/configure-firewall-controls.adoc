...
Weitere Informationen zur Verwendung von Firewall-Steuerelementen, einschließlich Beispielen, sind unter link:../admin/manage-firewall-controls.html["Management der Firewall-Kontrollen"] zu finden.

Weitere Informationen zu externen Firewalls und Netzwerksicherheit sind unter link:../admin/controlling-access-through-firewalls.html["Kontrolle des Zugriffs über externe Firewall"] zu finden.
...
NOTE: Die Einstellungen auf der Registerkarte „Privilegierte Adressliste“ können die Einstellungen auf der Registerkarte „Nicht vertrauenswürdiges Clientnetzwerk“ überschreiben.
...
. Optional kann *Weitere IP-Adresse oder ein Subnetz in CIDR-Notation hinzufügen* ausgewählt werden, um zusätzliche privilegierte Clients hinzuzufügen.
+

TIP: Fügen Sie so wenig Adressen wie möglich zur Liste der privilegierten Adressen hinzu.

. Optional kann *Privilegierten IP-Adressen der Zugriff auf interne StorageGRID Ports erlaubt werden*. Weitere Informationen unter link:../network/internal-grid-node-communications.html["Interne StorageGRID-Ports"].
+
...
CAUTION: Wenn Sie die Grid Manager Ports 443 oder 8443 schließen, verlieren alle Benutzer, die derzeit über einen blockierten Port verbunden sind, einschließlich Ihnen, den Zugriff auf Grid Manager, es sei denn, ihre IP-Adresse wurde der Liste der privilegierten Adressen hinzugefügt.
