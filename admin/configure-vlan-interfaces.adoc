...
summary: 'Sie können virtuelle LAN-Schnittstellen (VLAN) auf Admin-Knoten und Gateway-Knoten erstellen und diese in HA-Gruppen und Load-Balancer-Endpunkten verwenden, um den Datenverkehr aus Gründen der Sicherheit, Flexibilität und Leistung zu isolieren und zu partitionieren.' 
---
...
+
image:vlan-create-parent-interfaces.png["Übergeordnete Schnittstellen des Assistenten zur Erstellung von VLANs"]
...
** Wenn Sie eine übergeordnete Schnittstelle entfernen müssen, wählen Sie den Papierkorb image:icon-trash-can.png["Papierkorb-Symbol"] aus.
