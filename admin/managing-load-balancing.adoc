...
NOTE: Obwohl der StorageGRID Load Balancer-Dienst der empfohlene Lastausgleichsmechanismus ist, möchten Sie möglicherweise stattdessen einen Load Balancer eines Drittanbieters integrieren. Weitere Informationen erhalten Sie von Ihrem NetApp -Kundenbetreuer oder unter [Link einfügen]. https://docs.netapp.com/us-en/storagegrid-enable/technical-reports/load-balancer-index.html["Verwenden Sie Load Balancer von Drittanbietern mit StorageGRID."^]Die
...
Wenn S3-Anfragen im Virtual-Hosted-Style verwendet werden sollen, muss das Zertifikat auch einen *Alternative Name*-Eintrag für jede von Ihnen konfigurierte link:../admin/configuring-s3-api-endpoint-domain-names.html["Der Domänenname des S3-Endpunkts"] enthalten, einschließlich aller Platzhalternamen. Beispielsweise:
