...
summary: Grid-Federation-Verbindungen zwischen StorageGRID-Systemen können erstellt werden, wenn Mandantendetails synchronisiert und Objektdaten repliziert werden sollen. 
---
...
[role="lead"]
Grid-Federation-Verbindungen zwischen StorageGRID-Systemen können erstellt werden, wenn Mandantendetails synchronisiert und Objektdaten repliziert werden sollen.
...
TIP: Optional ist es möglich, link:../admin/grid-federation-automate-connections.html["die Erstellung, Bearbeitung und Wartung von Grid Federation-Verbindungen automatisieren"].

image:grid_federation_create_connection.png["Grafische Übersicht über die Schritte zum Erstellen einer Verbindung auf zwei Rastern"]
...
. Geben Sie einen Verbindungsnamen ein, der eindeutig ist und das Erkennen dieser Verbindung erleichtert. Zum Beispiel  `Grid 1-Grid 2`.
. Wenn Sie link:../admin/configure-global-namespace.html["Einen globalen Namensraum konfigurieren"] möchten, kann *Globalen Namensraum aktivieren* ausgewählt werden.
. Wählen Sie *Weiter*.
. Geben Sie die Grid-Details wie folgt ein:
...
* link:grid-federation-troubleshoot.html["Beheben Sie die Verbindungsherstellung"]einschließlich der Behebung von Warnungen und Fehlern im Zusammenhang mit der Kontosynchronisierung und der gridübergreifenden Replikation.
