...
[role="lead"]
Mit dem Konfigurationsassistenten für SSO für SAML kann in den Sandbox-Modus gewechselt werden, um Single Sign-On (SSO) zu konfigurieren und zu testen, bevor es für alle StorageGRID Benutzer aktiviert wird. Nach der Aktivierung von SSO besteht die Möglichkeit, bei Bedarf in den Sandbox-Modus zurückzukehren, um die Konfiguration zu ändern oder erneut zu testen.
...
Damit StorageGRID (der Dienstanbieter) und der SSO-Identitätsanbieter sicher über Benutzerauthentifizierungsanfragen kommunizieren können, sind folgende Aufgaben auszuführen:

. Konfigurieren Sie die Einstellungen in StorageGRID.
. Verwenden Sie die Software des SSO-Identitätsanbieters, um für jeden Admin-Knoten eine Vertrauensstellung der vertrauenden Seite (AD FS), eine Unternehmensanwendung (Entra ID) oder einen Dienstanbieter (PingFederate) zu erstellen.
...
. Unter *Protokoll* *SAML* auswählen.
...
== [[enter-sandbox-mode]]Schritt 2: Vertrauensstellungen der vertrauenden Seite, Unternehmensanwendungen oder SP Verbindungen bereitstellen

Füllen Sie die Felder in Schritt 2 des Assistenten entsprechend dem von Ihnen ausgewählten SSO-Typ aus.

[role="tabbed-block"]
====
.Active Directory
--
Füllen Sie die Felder im Schritt *Angabe der Kennung der vertrauenden Partei* aus.
...
.Entra-ID
--
Die Felder im Schritt *Unternehmensanwendungen konfigurieren* ausfüllen.
...
. Führen Sie die Schritte in link:../admin/creating-enterprise-application-entra-id-saml.html["Erstellen Sie Unternehmensanwendungen in Entra ID"] aus, um für jeden in der Tabelle aufgeführten Admin Node eine Enterprise-Anwendung zu erstellen.
...
.PingFederate
--
Die Felder im Schritt *SP-Verbindungs-ID angeben* ausfüllen.
...
== Schritt 3: Konfiguration von Vertrauensstellungen der vertrauenden Seite, Unternehmensanwendungen oder SP Verbindungen
...
+
Anweisungen sind unter link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html["Erstellen Sie Vertrauensstellungen von vertrauenswürdigen Parteien in AD FS"] zu finden.
...
. Führen Sie die Schritte in link:../admin/creating-enterprise-application-entra-id-saml.html["Erstellen Sie Unternehmensanwendungen in Entra ID"] aus, um die SAML-Metadatendatei für jeden Admin Node in die entsprechende Entra ID Enterprise Application hochzuladen.
...
. link:../admin/creating-sp-connection-ping-saml.html["Erstellen Sie eine oder mehrere SP-Verbindungen (Service-Provider) für StorageGRID"]. Die SP-Verbindungs-ID für jeden Admin-Knoten (angezeigt in der Tabelle auf der Seite „SSO konfigurieren“) und die SAML-Metadaten, die für diesen Admin-Knoten heruntergeladen wurden, sind zu verwenden.
+
...
== [[test-sso]]Schritt 4: Testkonfiguration
...
== [[enable-sso-saml]]Schritt 5: Single Sign-On aktivieren
...
Nach der Konfiguration von SSO mit SAML wird der Identitätsanbieter für die Admin-Nodes konfiguriert.

[cols="1a,2a"]
|===
| Wenn Ihr Identitätsanbieter... | Sie müssen... 


 a| 
Active Directory
 a| 
link:creating-relying-party-trusts-in-ad-fs-saml.html["Für jeden Admin-Knoten eine Anwendungsgruppe erstellen"]
...
Entra-ID
 a| 
link:creating-enterprise-application-entra-id-saml.html["Eine Anwendung für jeden Admin Node erstellen"]
...
PingFederate
 a| 
link:creating-sp-connection-ping-saml.html["Eine Dienstanbieterverbindung für jeden Admin Node erstellen"]
