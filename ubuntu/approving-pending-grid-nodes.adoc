...
.. Vergewissern Sie sich, dass Sie die entsprechenden Netzwerke konfigurieren können. Sie sollten bereits mit den Informationen ausgefüllt werden, die Sie auf der Seite IP-Konfiguration des Appliance Installer angegeben haben.
+
Informationen zur Installation von StorageGRID Appliances finden Sie unter, um die https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/index.html["Schnellstart für die Hardwareinstallation"^] Anweisungen für Ihr Gerät zu finden.
