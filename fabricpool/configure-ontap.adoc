...
URL-Stil
 a| 
Wenn Sie link:../admin/configuring-s3-api-endpoint-domain-names.html["Domänennamen des S3-Endpunkts wurden konfiguriert"], wählen Sie *Virtual-Hosted-Style URL* aus.

*Path-Style URL* ist die Standardeinstellung für ONTAP, jedoch wird für StorageGRID die Verwendung von virtual-hosted-style-Anfragen empfohlen. *Path-Style URL* muss verwendet werden, wenn im Feld *Server name (FQDN)* eine IP-Adresse anstelle eines Domänennamens angegeben wird.
