...
---
= Greifen Sie auf den StorageGRID FabricPool Einrichtungsassistenten zu und schließen Sie ihn ab
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Definieren Sie den Root-Zugriff für das Mandantenkonto, je nachdem, ob Ihr StorageGRID -Systemlink:../admin/using-identity-federation.html["Identitätsföderation"] ,link:../admin/how-sso-works.html["Single Sign On (SSO)"] oder beides.
+
[cols="1a,3a"]
|===
| Option | Tun Sie das 
...
.Schritte
. Optional kann das Kopiersymbol (image:icon_tenant_copy_url.png["Symbol kopieren"]) ausgewählt werden, um sowohl die Zugriffsschlüssel-ID als auch den geheimen Zugriffsschlüssel in die Zwischenablage zu kopieren.
+
