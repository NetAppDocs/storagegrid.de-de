...
summary: 'Bei der Konfiguration von StorageGRID für die Verwendung mit FabricPool können optional eine oder mehrere Hochverfügbarkeitsgruppen (HA-Gruppen) erstellt werden.' 
---
= Erstellen Sie eine Hochverfügbarkeitsgruppe (HA) für FabricPool in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
