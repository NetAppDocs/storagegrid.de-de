...
.Bevor Sie beginnen
* Sie haben die Informationen in link:../network/index.html["Netzwerkrichtlinien"] überprüft.
...
Durch das Klonen der MAC-Adresse verwendet der Container die MAC-Adresse des Hosts, und der Host verwendet die MAC-Adresse entweder einer von Ihnen angegebenen Adresse oder einer zufällig generierten.
...
In bestimmten Umgebungen kann die Sicherheit durch MAC-Adressklonen erhöht werden, da dies die Verwendung einer dedizierten virtuellen Netzwerkkarte für das Admin Network, Grid Network und Client Network ermöglicht.
...
* MAC-Klonen nicht aktiviert: Wenn der  `_CLONE_MAC` Schlüssel in der Konfigurationsdatei nicht gesetzt oder auf „false“ gesetzt ist, verwendet der Host die MAC-Adresse der Host-NIC und der Container erhält eine von StorageGRID generierte MAC-Adresse, sofern im  `_NETWORK_MAC` Schlüssel keine MAC-Adresse angegeben ist. Ist im  `_NETWORK_MAC` Schlüssel eine Adresse angegeben, erhält der Container die im  `_NETWORK_MAC` Schlüssel angegebene Adresse.
* MAC-Klonen aktiviert: Wenn der  `_CLONE_MAC` Schlüssel in der Konfigurationsdatei auf „true“ gesetzt ist, verwendet der Container die MAC-Adresse der Host-Netzwerkkarte, und der Host verwendet eine von StorageGRID generierte MAC-Adresse, sofern im  `_NETWORK_MAC` Schlüssel keine MAC-Adresse angegeben ist. Wenn im  `_NETWORK_MAC` Schlüssel eine Adresse angegeben ist, verwendet der Host die angegebene Adresse anstelle einer generierten.



NOTE: Wenn Sie die MAC-Adressenklonierung nicht verwenden möchten und stattdessen allen Schnittstellen erlauben möchten, Daten für andere MAC-Adressen als die vom Hypervisor zugewiesenen zu empfangen und zu senden, muss sichergestellt sein, dass die Sicherheitseigenschaften auf Ebene des virtuellen Switch und der Portgruppe für MAC-Adressänderungen und Forged Transmits auf *Akzeptieren* gesetzt sind. Die auf dem virtuellen Switch festgelegten Werte können durch die Werte auf Portgruppenebene überschrieben werden, daher sollte sichergestellt sein, dass die Einstellungen an beiden Stellen identisch sind.
...
image:rhel_install_vlan_diag_1.gif["VLAN-Diagramm installieren"]
...
image:rhel_install_vlan_diag_2.gif["VLAN-Schnittstellen"]
