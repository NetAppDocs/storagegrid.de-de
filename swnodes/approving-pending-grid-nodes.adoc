...
---
= Ausstehende Grid-Knoten für StorageGRID genehmigen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:5_gmi_installer_grid_nodes_pending.gif["Dieses Bild wird durch den umgebenden Text erläutert."]
...
.. Zurück zur Startseite und *Installation starten* auswählen.
...
+
image:7_gmi_installer_grid_nodes_approved.gif["Dieses Bild wird durch den umgebenden Text erläutert."]

. Wiederholen Sie diese Schritte für jeden ausstehenden Rasterknoten, den Sie genehmigen möchten.
+
Sie müssen alle Knoten, die Sie im Grid haben möchten, genehmigen. Es ist jedoch möglich, jederzeit zu dieser Seite zurückzukehren, bevor auf der Übersichtsseite *Install* ausgewählt wird. Die Eigenschaften eines genehmigten Grid-Knotens lassen sich ändern, indem dessen Optionsfeld ausgewählt und anschließend *Edit* gewählt wird.

. Wenn Sie die Grid-Knoten genehmigt haben, wählen Sie *Weiter*.
