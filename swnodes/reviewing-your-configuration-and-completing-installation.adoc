...
---
= Überprüfen Sie Ihre StorageGRID-Konfiguration
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:11_gmi_installer_summary_page.gif["Die Seite mit der Übersicht zum Grid Manager Installer"]
...
NOTE: Wenn ein Knoten für die Verwendung des Client-Netzwerks konfiguriert ist, wechselt das Standardgateway dieses Knotens beim Auswählen von *Install* vom Grid-Netzwerk zum Client-Netzwerk. Bei einem Verbindungsverlust ist sicherzustellen, dass auf den primären Admin-Knoten über ein erreichbares Subnetz zugegriffen wird. Siehe link:../network/index.html["Netzwerkrichtlinien"] für Details.
...
. Das Kontrollkästchen *Ich habe die Wiederherstellungspaketdatei erfolgreich heruntergeladen und überprüft* auswählen und *Weiter* wählen.
+
...
+
image:12_gmi_installer_status_page.gif["Dieses Bild wird durch den umgebenden Text erläutert."]
