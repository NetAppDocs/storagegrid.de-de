...
---
= Beheben von Problemen mit dem Admin Node und der Benutzeroberfläche in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Wenn Single Sign-On (SSO) für Ihr StorageGRID System aktiviert ist, finden Sie Informationen zur Einrichtung von SSO mit link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] oder link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
+
...
... *Verbindung testen* auswählen, um die Verbindungseinstellungen für den LDAP-Server zu validieren.
...
. Verwenden Sie Secure Shell (SSH), um sich am Admin-Knoten anzumelden:
+
