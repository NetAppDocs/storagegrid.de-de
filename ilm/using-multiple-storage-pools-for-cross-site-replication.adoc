...
CAUTION: Wenn Sie Ihre StorageGRID-Bereitstellung für den Schutz vor Standortausfällen konfigurieren, müssen Sie auch die Auswirkungen von  und berücksichtigenlink:data-protection-options-for-ingest.html["Aufnahmeoptionen"]link:../s3/consistency.html["Konsistenz"].
