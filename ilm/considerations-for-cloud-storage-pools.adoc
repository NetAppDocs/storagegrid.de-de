...
* Im Allgemeinen ist Cloud-Archivspeicher wie Amazon S3 Glacier oder Azure Blob Storage ein kostengünstiger Ort zur Speicherung von Objektdaten. Die Kosten für das Abrufen von Daten aus Cloud-Archivspeicher sind jedoch relativ hoch. Um die niedrigsten Gesamtkosten zu erreichen, sollte berücksichtigt werden, wann und wie oft auf die Objekte im Cloud Storage Pool zugegriffen wird. Die Nutzung eines Cloud Storage Pool wird nur für Inhalte empfohlen, auf die voraussichtlich selten zugegriffen wird.
...
Die Azure Blob Storage Lifecycle-Managementfunktion sollte nicht für den Container verwendet werden, der mit einem Cloud Storage Pool genutzt wird. Lebenszyklusvorgänge können mit den Vorgängen des Cloud Storage Pools in Konflikt geraten.
