...
+
Wenn das Objekt mit einer ILM-Regel übereinstimmt, die einen Azure Cloud Storage Pool als Ablageort verwendet, verschiebt StorageGRID das Objekt in den externen Azure Blob Storage Container, der vom Cloud Storage Pool angegeben wird.

. *Objekt in Archivebene (nicht-Retrieable-Status)* umgestiegen
+
Unmittelbar nach dem Verschieben des Objekts in den Azure Cloud Storage Pool verschiebt StorageGRID das Objekt automatisch in die Azure Blob Storage Archive-Ebene.
...
+
Wenn StorageGRID die RestoreObject empfängt, wird das Objekt vorübergehend in die Azure Blob Storage Cool-Ebene verschoben. Sobald das Ablaufdatum in der RestoreObject-Anfrage erreicht ist, verschiebt StorageGRID das Objekt zurück in die Archive-Ebene.
