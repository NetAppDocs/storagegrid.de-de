...
* `Content-Encoding`
+
Wenn  `aws-chunked` für  `Content-Encoding` angegeben wird, überprüft StorageGRID die folgenden Elemente nicht:
...
NOTE: Wenn in einer PutObject-Anfrage keine Prüfsumme angegeben wird, berechnet StorageGRID keine Standard-CRC-64/NVME-Prüfsumme für ein Objekt.
