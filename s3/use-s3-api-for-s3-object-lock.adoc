...
NOTE: StorageGRID unterstützt den  `x-amz-sdk-checksum-algorithm` Anfrageheader nicht.
...
Um die S3-Objektsperre für einen Bucket zu aktivieren, kann eine der folgenden Methoden genutzt werden:
...
* Bucket-Einstellungen können im Tenant Manager verwaltet werden. Weitere Informationen unter link:../tenant/update-default-retention-settings.html["Aktualisieren Sie die S3 Object Lock-Standardaufbewahrung"].
...
Einer der folgenden Anfrageheader ist in der PutObjectLockConfiguration-Anfrage anzugeben:
...
NOTE: StorageGRID validiert den Inhalt des x-amz-checksum-* Headers für diese Operation nicht.



==== Anforderungsbeispiel

Dieses Beispiel aktiviert S3 Object Lock für einen Bucket (sofern Ihr Grid-Administrator die Berechtigung erteilt hat) und legt den Standard-Aufbewahrungsmodus auf COMPLIANCE und die Standard-Aufbewahrungsdauer auf 6 Jahre fest.
...
* Einer der folgenden Anfrageheader muss angegeben werden, wenn ein x-amz-object-lock-* Anfrageheader in der PutObject Anfrage vorhanden ist:
+
[listing]
----
Content-MD5
...
----
+
Diese Header sind für CopyObject oder CreateMultipartUpload nicht erforderlich.

* Wenn für den Bucket die S3 Object Lock-Funktion nicht aktiviert ist und ein x-amz-object-lock-* Anforderungsheader vorhanden ist, wird ein 400 Bad Request (InvalidRequest) Fehler zurückgegeben.
...
Einer der folgenden Anfrageheader muss in der Anfrage angegeben werden:
...
NOTE: StorageGRID validiert bei diesen Operationen nicht den Inhalt des x-amz-checksum-* Headers.
...
+
Der  `x-amz-bypass-governance-retention`Header hat keine Auswirkung auf Objekte ohne S3 Object Lock.
