...
summary: Das StorageGRID System unterstützt maximal 20.000 Buckets für jedes S3-Mandantenkonto. 
---
...
[role="lead"]
Das StorageGRID System unterstützt maximal 20.000 Buckets für jedes S3-Mandantenkonto.

Jedes Grid kann maximal 100,000 Buckets enthalten.

Wenn mehrere Mandanten jeweils bis zu 5.000 Buckets haben, muss jeder Storage Node im Grid über mindestens 64 GB RAM verfügen.

Die Beschränkungen für Bucket-Namen orientieren sich an den Beschränkungen der AWS US Standard-Region, sollten aber zusätzlich auf DNS-Namenskonventionen beschränkt werden, um S3 Virtual-Hosted-Style-Anfragen zu unterstützen.

Weitere Informationen sind im Folgenden zu finden:
...
Sie können überprüfen, ob Aktualisierungen der letzten Zugriffszeit für einzelne Buckets aktiviert oder deaktiviert sind. Weitere Informationen finden Sie unter link:get-bucket-last-access-time-request.html["ZEITPUNKT des letzten Zugriffs FÜR den Bucket ABRUFEN"].
...
*Hinweis*: S3-Verzeichnis-Buckets werden nicht unterstützt.
...
+
*Hinweis*: Es tritt ein Fehler auf, wenn Ihre CreateBucket-Anfrage eine Region verwendet, die in StorageGRID nicht definiert wurde.

* Sie können den  `x-amz-bucket-object-lock-enabled` Anforderungsheader einfügen, um einen Bucket mit aktivierter S3 Object Lock zu erstellen. Weitere Informationen finden Sie unter link:../s3/use-s3-api-for-s3-object-lock.html["Konfigurieren Sie die S3-Objektsperre über die S3-REST-API"].
+
S3 Object Lock erfordert die Bucket-Versionierung, die automatisch aktiviert wird, wenn der Bucket mit aktiviertem S3 Object Lock erstellt wird.

+
Sie können die S3 Object Lock beim Erstellen des Buckets oder zu einem späteren Zeitpunkt aktivieren.
...
*Achtung*: Wenn für diesen Bucket ein nicht standardmäßiges ILM-Richtlinien-Tag festgelegt ist, gibt es ein  `NTAP-SG-ILM-BUCKET-TAG` Bucket-Tag mit einem zugewiesenen Wert. Eine DeleteBucketTagging-Anfrage sollte nicht gestellt werden, wenn ein  `NTAP-SG-ILM-BUCKET-TAG` Bucket-Tag vorhanden ist. Stattdessen sollte eine PutBucketTagging-Anfrage mit nur dem  `NTAP-SG-ILM-BUCKET-TAG` Tag und dessen zugewiesenem Wert gestellt werden, um alle anderen Tags aus dem Bucket zu entfernen. Das  `NTAP-SG-ILM-BUCKET-TAG` Bucket-Tag sollte nicht geändert oder entfernt werden.
...
GetBucketLocation
 a| 
Gibt die Region zurück, die mit dem  `LocationConstraint` Element in der CreateBucket-Anfrage festgelegt wurde. Wenn die Region des Buckets  `us-east-1` (Standard) ist, wird für  `LocationConstraint` ein leerer Wert zurückgegeben.
...
*Achtung*: Wenn für diesen Bucket ein nicht standardmäßiges ILM-Richtlinien-Tag festgelegt ist, gibt es ein  `NTAP-SG-ILM-BUCKET-TAG` Bucket-Tag mit einem zugewiesenen Wert. Dieses Tag sollte nicht geändert oder entfernt werden.
...
Weitere Informationen finden Sie unter link:../s3/use-s3-api-for-s3-object-lock.html["Konfigurieren Sie die S3-Objektsperre über die S3-REST-API"].
...
Siehe link:create-s3-lifecycle-configuration.html["S3-Lebenszykluskonfiguration erstellen"]. Weitere Informationen darüber, wie die Ablaufaktion im Lebenszyklus eines Buckets mit ILM-Platzierungsanweisungen interagiert, finden Sie unter link:../ilm/how-ilm-operates-throughout-objects-life.html["Wie ILM im gesamten Leben eines Objekts funktioniert"].
...
PutBucketPolicy
 a| 
Legt die dem Bucket zugeordnete Richtlinie fest. Siehe link:use-access-policies.html["Verwendung von Bucket- und Gruppenzugriffsrichtlinien"].
...
* StorageGRID unterstützt ausschließlich die Replikationskonfiguration V1. Das bedeutet, dass StorageGRID die Verwendung des  `Filter` Elements für Regeln nicht unterstützt und die V1-Konventionen für das Löschen von Objektversionen anwendet. Weitere Informationen finden sich unter  https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication-add-config.html["Amazon Simple Storage Service User Guide: Replizierungskonfiguration"^].
...
* Ziel-Buckets müssen als URN von StorageGRID Endpunkten angegeben werden, wie im Tenant Manager oder der Tenant Management API definiert. Siehe link:../tenant/configuring-cloudmirror-replication.html["CloudMirror-Replizierung konfigurieren"].
+
...
*Achtung*: Wenn für diesen Bucket ein nicht standardmäßiges ILM-Policy-Tag festgelegt ist, gibt es ein  `NTAP-SG-ILM-BUCKET-TAG` Bucket-Tag mit einem zugewiesenen Wert. Es ist sicherzustellen, dass das  `NTAP-SG-ILM-BUCKET-TAG` Bucket-Tag mit dem zugewiesenen Wert in allen PutBucketTagging-Anfragen enthalten ist. Dieses Tag sollte weder geändert noch entfernt werden.
...
* Wenn der Grid-Administrator die Berechtigung erteilt hat, ermöglicht dies die Aktivierung der S3 Object Lock-Funktion für bestehende Buckets.


Wird die Standardaufbewahrungsfrist geändert, bleibt das Aufbewahrungsdatum für bestehende Objektversionen unverändert und wird nicht anhand der neuen Standardaufbewahrungsfrist neu berechnet.

Ausführliche Informationen sind unter link:../s3/use-s3-api-for-s3-object-lock.html["Konfigurieren Sie die S3-Objektsperre über die S3-REST-API"] zu finden.
