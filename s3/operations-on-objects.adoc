...
* Die `If-Match` und `If-None-Match` Header verhalten sich je nach StorageGRID Konfiguration unterschiedlich. Das Angeben dieser Header kann eine Fehlermeldung (501 Not Implemented) zurückgeben. Werden die `If-Match` oder `If-None-Match` Header zwar akzeptiert, sind aber nicht funktionsfähig, kann eine Warnung mit Hinweisen zur Aktivierung des strengeren 501 Not Implemented Fehlerverhaltens ausgelöst werden.
