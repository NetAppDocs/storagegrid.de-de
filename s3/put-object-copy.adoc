...
StorageGRID analysiert oder interpretiert keine maskierten UTF-8-Zeichen, die im Schlüsselnamen oder Wert benutzerdefinierter Metadaten enthalten sind. Maskierte UTF-8-Zeichen werden als ASCII-Zeichen behandelt.
...
* `Content-Type`
* `x-amz-checksum-algorithm`: Geben Sie  `CRC32`, `CRC32C`, `CRC64NVME` oder `SHA256` an
+
Wenn ein Objekt kopiert wird und das Quellobjekt eine  `checksum` oder  `x-amz-checksum-algorithm` enthält, erhält das kopierte Objekt eine FULL_OBJECT-Prüfsumme.

+
Wenn das Quellobjekt eine Prüfsumme hat, die Anfrage jedoch keine  `x-amz-checksum-algorithm`enthält oder einen nicht unterstützten Prüfsummenwert verwendet, hat das kopierte Objekt denselben Prüfsummtyp wie das Quellobjekt. StorageGRID ignoriert ungültige oder nicht unterstützte  `x-amz-checksum-algorithm`Werte.

+
Wenn das Quellobjekt keine Prüfsumme hat und  `x-amz-checksum-algorithm` nicht enthält oder einen nicht unterstützten Prüfsummenwert verwendet, besitzt das kopierte Objekt keine Prüfsumme.

+

NOTE: Wenn in einer CopyObject-Anfrage keine Prüfsumme angegeben ist und das Quellobjekt keine Prüfsumme hat, berechnet StorageGRID keine Standard-CRC-64/NVME-Prüfsumme für ein Objekt.
...
* Die in der entsprechenden ILM-Regel festgelegte Option für das Aufnahmeverhalten wird nicht verwendet. Alle durch das Update ausgelösten Änderungen an der Objektplatzierung werden vorgenommen, wenn ILM durch normale Hintergrundprozesse erneut ausgewertet wird.
+
