...
Diese Seite enthält nur die von StorageGRID unterstützten S3-Operationen. Alle nicht aufgeführten S3-Operationen können  `501 Not Implemented` zurückgeben oder zu unbeabsichtigtem Verhalten führen.


TIP: Um die AWS Dokumentation für jeden Vorgang anzuzeigen, klicken Sie in der Überschrift auf den Link.
...
.URI-Abfrageparameter und Anforderungskopfzeilen
StorageGRID unterstützt _die meisten_ <<common-params,Allgemeine Parameter und Kopfzeilen>> für diese Anfrage.

.Text anfordern
StorageGRID unterstützt _die meisten_ Anfragekörperparameter, die von der Amazon S3 REST API zum Zeitpunkt der Implementierung definiert sind, mit folgender Ausnahme:

Sie können PutObjectLockConfiguration nicht verwenden, um S3 Object Lock für einen bestehenden Bucket zu aktivieren, es sei denn, Ihr Grid-Administrator hat die entsprechende Berechtigung erteilt.
