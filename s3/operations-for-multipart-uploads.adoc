...
* Ob ein mehrteilig hochgeladenes Objekt in eine link:../tenant/what-is-branch-bucket.html["Asteimer"] aufgenommen wird, hängt vom Zeitpunkt des Ingest-Starts und dem Zeitfilter für den Branch-Bucket ab:
+
** *Vorher*. Wenn die Aufnahme vor dem _Vorher_-Zeitstempel des Branch-Buckets initiiert wurde, wird das mehrteilige hochgeladene Objekt unabhängig vom Zeitpunkt des Abschlusses des Uploads einbezogen.
** *Nach*. Wenn die Aufnahme nach dem _Nach_-Zeitstempel des Branch-Buckets initiiert wurde, wird das mehrteilig hochgeladene Objekt einbezogen.
** *Zwischen*. Wenn die Aufnahme zwischen den Zeitstempeln _From_ und _To_ des Branch Buckets initiiert wurde, wird das mehrteilig hochgeladene Objekt unabhängig vom Zeitpunkt des Abschlusses des Uploads einbezogen.
** *Ausschließen*. Wenn die Aufnahme vor dem _Ausschluss-von_ Zeitpunkt des Branch-Buckets oder nach dem _Ausschluss-bis_ Zeitpunkt initiiert wurde, wird das mehrteilige hochgeladene Objekt eingeschlossen.
