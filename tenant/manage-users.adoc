...
summary: 'Sie können lokale Benutzer erstellen und diese lokalen Gruppen zuweisen, um festzulegen, auf welche Funktionen diese Benutzer zugreifen können. Der Tenant Manager enthält einen vordefinierten lokalen Benutzer mit dem Namen `root`, obwohl Sie lokale Benutzer hinzufügen und entfernen können, kann der Root-Benutzer nicht entfernt werden.' 
---
