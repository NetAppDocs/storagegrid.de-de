...
summary: Sie können lokale Benutzer erstellen und sie lokalen Gruppen zuweisen, um festzulegen, auf welche Funktionen diese Benutzer zugreifen können. Der Tenant Manager enthält einen vordefinierten lokalen Benutzer namens root. Obwohl Sie lokale Benutzer hinzufügen und entfernen können, kann der Root-Benutzer nicht entfernt werden. 
---
...
+
image:grid-federation-tenant-user-banner.png["Abbildung des blauen Banners auf der Benutzerseite, wenn es sich um das Quellenraster des Mandanten handelt"]
