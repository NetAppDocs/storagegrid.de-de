...
summary: Wenn Ihr StorageGRID-Mandantenkonto über die Berechtigung Use grid federation connection verfügt, können Sie die Tenant Management API verwenden, um S3-Zugriffsschlüssel manuell zu klonen. 
---
...
+
image:grid-federation-post-current-user-replicate.png["POST-Endpunkt für die Replikation des eigenen Zugriffsschlüssels"]
...
+
image:grid-federation-clone-access-key.png["Die Mandanten-Manager-API gibt Werte zum Klonen des Zugriffsschlüssels ein"]
...
+
image:grid-federation-post-other-user.png["POST-Endpunkt zur Replikation des Zugriffsschlüssels eines anderen Benutzers"]
