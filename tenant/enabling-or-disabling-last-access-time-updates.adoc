...
summary: 'Wenn Grid-Administratoren die Information Lifecycle Management (ILM)-Regeln für ein StorageGRID System erstellen, kann optional festgelegt werden, dass die letzte Zugriffszeit eines Objekts verwendet wird, um zu bestimmen, ob dieses Objekt an einen anderen Speicherort verschoben werden soll. Wenn ein S3-Tenant verwendet wird, kann von solchen Regeln profitiert werden, indem die Aktualisierung der letzten Zugriffszeit für die Objekte in einem S3-Bucket aktiviert wird.' 
---
= Aktivieren oder deaktivieren Sie Aktualisierungen der letzten Zugriffszeit in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| Zeitpunkt des letzten Zugriffs aktualisiert? | Das Objekt wurde zur ILM-Auswertungswarteschlange hinzugefügt? | Zeitpunkt des letzten Zugriffs aktualisiert? | Das Objekt wurde zur ILM-Auswertungswarteschlange hinzugefügt? 
 ...
 a| 
Anforderung zum Abrufen der Metadaten eines Objekts, wenn eine HEAD-Operation ausgeführt wird
 a| 
Nein
 a| 
Nein
