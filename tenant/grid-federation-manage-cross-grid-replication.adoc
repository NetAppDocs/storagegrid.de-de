...
summary: Wenn Ihrem Mandantenkonto bei der Erstellung die Berechtigung Use grid federation connection zugewiesen wurde, können Sie die Grid-übergreifende Replikation nutzen, um Objekte automatisch zwischen Buckets im Quell-Grid des Mandanten und Buckets im Ziel-Grid des Mandanten zu replizieren. Die Grid-übergreifende Replikation kann in eine oder beide Richtungen erfolgen. 
---
= Verwalten der gridübergreifenden Replikation in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:grid-federation-cgr-workflow.png["Workflow für Grid-übergreifende Replizierung"]
...
+
image:grid-federation-cross-grid-replication-client-deletes.png["Abbildung zeigt, wie der Replikate-Client auf beiden Rastern gelöscht wird"]
