...
+
image:tenant_accounts_page.png["Seite „Mandantenkonten“"]

. Optional ist eine Anmeldung bei einem Mandantenkonto möglich, indem der Sign in-Link image:icon_tenant_sign_in.png["Symbol für die Anmeldung beim Mandanten"] in der Spalte *Sign in/Copy URL* ausgewählt wird.
. Optional kann die URL der Sign in-Seite eines Mandanten kopiert werden, indem der Link „URL kopieren image:icon_tenant_copy_url.png["URL-Symbol für das Kopieren von Mandanten"]“ in der Spalte *Sign in/Copy URL* ausgewählt wird.
...
+
image:tenant_accounts_example_csv.png["Mandantenkonten, Beispiel .csv"]
...
+
image:tenant_usage_modal.png["Nutzung von Mandanten modal"]
...
+
image:tenant_bucket_space_consumption_GM.png["Balkendiagramm für die Bucket-Nutzung von Mandanten"]
...
+
image:tenant_bucket_details_csv.png["Beispiel für die Bucket-Details des Mandanten (CSV"]
