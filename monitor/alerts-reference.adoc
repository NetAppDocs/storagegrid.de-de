...
Appliance-CMOS-Batteriefehler
 a| 
Es wurde ein Problem mit einer CMOS-Batterie im Gerät festgestellt.
...
Die SANtricity -Softwareversion ist niedriger als das empfohlene Minimum für diese Version von StorageGRID.
 ...
 a| 
Appliance SFP-Fehler erkannt
 a| 
Im Compute-Controller eines StorageGRID Geräts wurde ein SFP-Fehler festgestellt.
...
In einem FabricPool Bucket ist die Versionierung oder die S3-Objektsperrung aktiviert, die nicht unterstützt werden.
 ...
 a| 
ListBucket-Berechtigungsprüfung für Get/Head Objekt deaktiviert
 a| 
ListBucket-Berechtigungsprüfung für GetObject/HeadObject bei nicht vorhandenen Schlüsseln ist deaktiviert.
...
| Alarmname | Beschreibung 
 ...
 a| 
Chunk Service mit hoher Goroutinenanzahl
 a| 
Der Chunk Service weist eine ungewöhnlich hohe Anzahl von Goroutinen auf, was auf einen Deadlock oder ein Ressourcenleck hindeuten könnte.
...
Fragmente, die mit der Löschung codiert wurden, können nicht mehr verifiziert werden. Beschädigte Fragmente werden möglicherweise nicht repariert.
 ...
 a| 
Löschcodierte Gruppen im Status „Reparaturbedürftig“
 a| 
Eine oder mehrere Löschcodierungsgruppen befinden sich im Status "Reparatur erforderlich", wenn keine Löschcodierungsdatenreparatur im Gange ist.
...
Es wurden verwaiste Objekte erkannt.
 ...
 a| 
S3 Multipart Upload Teilgröße zu groß
 a| 
Ein Client versucht einen mehrteiligen Upload-Vorgang, der die Teilgrößenbeschränkung überschreitet.
...
Ein Client versucht, eine PUT-Objekt-Operation durchzuführen, die die S3-Größenlimits überschreitet.
 ...
 a| 
S3-Anfragen mit nicht unterstützten bedingten Headern
 a| 
Ein S3 Client verwendet für bestimmte Vorgänge nicht unterstützte bedingte Header (If-Match oder If-None-Match).



 a| 
S3-Anfragen mit falscher Region bei aktiviertem globalem Namensraum
 a| 
Ein S3-Client verwendet die falsche Signaturregion, um auf die Buckets dieses Grids zuzugreifen.
