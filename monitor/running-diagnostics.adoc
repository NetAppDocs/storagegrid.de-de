...
---
= Führen Sie Diagnose in StorageGRID aus
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"] *Normal*: Alle Werte liegen im Normalbereich.
* image:icon_alert_yellow_minor.png["Symbol Alarm Gelb Minor"] *Achtung*: Mindestens ein Wert liegt außerhalb des normalen Bereichs.
* image:icon_alert_red_critical.png["Symbol Warnung Rot Kritisch"]*Vorsicht*: Einer oder mehrere der Werte liegen deutlich außerhalb des Normbereichs.
...
+
image:support_diagnostics_page.png["Support Diagnostics Seite"]

. Weitere Informationen zu einer bestimmten Diagnose sind durch Auswahl der entsprechenden Zeile verfügbar.
+
...
+
image:support_diagnostics_cpu_utilization.png["Beispiel für Diagnosestatuswerte"]
...
+
image:grafana_dashboard_nodes.png["Grafana Dashboard-Knoten"]

. *Optional*: Ein Diagramm des Prometheus-Ausdrucks im Zeitverlauf ist über *In Prometheus anzeigen* einsehbar.
+
Es wird ein Prometheus-Diagramm des in der Diagnose verwendeten Ausdrucks angezeigt.

+
image:support_diagnostics_prometheus_png.png["Support Diagnostics Prometheus Seite"]
