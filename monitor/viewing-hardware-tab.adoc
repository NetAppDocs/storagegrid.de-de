...
---
= Registerkarte „StorageGRID-Hardware“ anzeigen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:nodes_page_hardware_tab_graphs.png["Registerkarte Knoten Hardware"]
...
image:nodes_page_memory_usage_details.png["Knotenseite > Hardware > Speichernutzungsdetails"]
...
+
image:nodes_page_overview_tab_extended.png["Knotenübersicht Erweitert"]
...
+
.. Die Diagramme zur CPU-Auslastung und Speichernutzung zeigen die prozentualen Werte der CPU- und Speichernutzung im Zeitverlauf. Zur Anzeige eines anderen Zeitintervalls steht eines der Steuerelemente oberhalb des Diagramms oder der Grafik zur Auswahl. Die verfügbaren Informationen können für Intervalle von 1 Stunde, 1 Tag, 1 Woche oder 1 Monat angezeigt werden. Ein benutzerdefiniertes Intervall kann ebenfalls festgelegt werden, wodurch sich Datums- und Zeitbereiche angeben lassen.
+
image:nodes_page_hardware_tab_graphs.png["Hardware-Grafiken"]
...
Der Temperaturstatus des Compute-Controllers.
 ...
 a| 
Stromversorgung des Compute Controllers A
 a| 
Der Status des Netzteils A für den Rechencontroller.



 a| 
Stromversorgung des Compute Controllers B
 a| 
Der Status des Netzteils B für den Rechencontroller.
...
+
image:nodes_page_network_traffic_graph.png["Knoten Seite Netzwerk Verkehr Diagramm"]

+
.. Der Abschnitt Netzwerkschnittstellen sollte überprüft werden.
+
image:nodes_page_network_interfaces.png["Knoten Seite Netzwerkschnittstellen"]

+
Anhand der folgenden Tabelle und der Werte in der Spalte *Speed* in der Tabelle „Netzwerkschnittstellen“ kann ermittelt werden, ob die 10/25-GbE Netzwerkports des Geräts für den active/backup-Modus oder den LACP-Modus konfiguriert wurden.
...
.. Der Abschnitt Netzwerkkommunikation kann eingesehen werden.
+
...
+
image:nodes_page_storage_used_object_data.png["Speicher Verwendet - Objektdaten"]

+
image:storage_used_object_metadata.png["Verwendeter Storage: Objekt-Metadaten"]
...
+
image:nodes_page_overview_tab_services_appliance.png["Registerkarte Knoten der Registerkarte Überblick für Services Appliance"]
...
. Die Registerkarte *Hardware* zeigt weitere Informationen zum Gerät.
+
...
+
image:nodes_page_hardware_tab_graphs_services_appliance.png["Knoten Seite Hardware Registerkarte Diagramme für Services Appliance"]

.. Scrollen Sie nach unten, um die Komponententabelle für das Gerät anzuzeigen. Die Tabelle enthält Informationen wie den Modellnamen, die Seriennummer, die Controller-Firmware-Version und den Status jeder Komponente.
+
[cols="1a,2a"]
|===
| Feld in der Appliance-Tabelle | Beschreibung 
...
. Die Registerkarte *Netzwerk* zeigt Informationen zu jedem Netzwerk an.
+
Das Diagramm „Netzwerkverkehr“ bietet eine Zusammenfassung des gesamten Netzwerkverkehr.
...
.. Der Abschnitt Netzwerkschnittstellen sollte überprüft werden.
+
Anhand der folgenden Tabelle und der Werte in der Spalte *Speed* in der Tabelle „Netzwerkschnittstellen“ lässt sich feststellen, ob die vier 40/100-GbE Netzwerkports des Geräts für den Active/Backup-Modus oder den LACP-Modus konfiguriert wurden.
...
. Die Registerkarte *Speicher* zeigt Informationen über die Datenträger und Volumes auf der Services Appliance an.
