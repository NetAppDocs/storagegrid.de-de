...
NOTE: Für SNMPv3-Trap-Ziele wird empfohlen, für jeden Admin-Knoten einen USM-Benutzer zu erstellen.  Wenn nicht jeder Admin-Knoten über einen USM-Benutzer verfügt, erhält Ihr Verwaltungssystem möglicherweise keine Benachrichtigungen mehr, wenn der primäre Admin-Knoten ausfällt.
