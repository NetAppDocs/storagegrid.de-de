...
| acct-tunnel.log  a| 
Protokoll für den Account Tunnel Helper, der mit dem Load Balancer zusammenarbeitet.
...
Internes HTTP-Proxy-Verkehrsprotokoll.
...
| manage-audit.log  a| 
`mgmt-api`Auf Überwachungsprotokolle zugreifen.
...
* stream-access.log: TCP/TLS-Stream (Schicht 4) Proxy-Sitzungen für den Stream-Block.
+
*Hinweis*: Dies ist kein HTTP-Anforderungsprotokoll.
...
Zugriffsprotokoll für Grid-Federation-Datenverkehr, der wie S3 (globaler Namespace/Federation-URLs) geroutet wird.
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`: System-Bucket-Protokolle.
...
`update-cors-configurations.log`: Protokolle im Zusammenhang mit TLS/Verschlüsselungs- und CORS-Änderungen.



| update-leakd-configurations.log  a| 
Konfigurationsprotokoll zur Erkennung von Orphans.
