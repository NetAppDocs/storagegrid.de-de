...
Informationen zum Exportieren der Protokolle finden Sie unter link:../monitor/configure-audit-messages.html["Konfigurieren von Audit-Meldungen und externem Syslog-Server"].
...
Im Zusammenhang mit der Aufrechterhaltung der Konsistenz des neuesten Wiederherstellungspakets über alle Admin-Knoten und Speicher-Nodes, die den ADC-Dienst hosten.
