...
summary: Die S3 REST API, die von StorageGRID verwendet wird, wird regelmäßig aktualisiert. 
---
= Änderungen an StorageGRID S3 REST API Unterstützung
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Die S3 REST API, die von StorageGRID verwendet wird, wird regelmäßig aktualisiert.
...
* `If-Match`
+
Der  `If-Match`Header verhält sich je nach Ihrer StorageGRID Konfiguration unterschiedlich. Das Bereitstellen dieses Headers kann eine Fehlermeldung (501 Not Implemented) zurückgeben. Wenn der  `If-Match`Header akzeptiert, aber nicht funktionsfähig ist, kann eine Warnung mit Hinweisen zur Aktivierung des strengeren 501 Not Implemented Fehlerverhaltens ausgelöst werden.

* `If-None-Match`
+
Der  `If-None-Match`Header verhält sich je nach Ihrer StorageGRID Konfiguration unterschiedlich. Das Bereitstellen dieses Headers kann eine Fehlermeldung (501 Not Implemented) zurückgeben. Wenn der  `If-None-Match`Header akzeptiert, aber nicht funktionsfähig ist, kann eine Warnung mit Hinweisen zur Aktivierung des strengeren 501 Not Implemented Fehlerverhaltens ausgelöst werden.

* Unterstützung für vorab berechnete Prüfsummenwerte für die folgenden Anfragen und unterstützten Header hinzugefügt:
+
** CompleteMultipartUpload: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c` und `x-amz-checksum-crc64nvme`
** CreateMultipartUpload: `CRC32`, `CRC32C`, und `CRC64NVME` für `x-amz-checksum-algorithm`
** CopyObject: `CRC32`, `CRC32C`, `CRC64NVME`, und `SHA256` für `x-amz-checksum-algorithm`
** PutObject: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, und `x-amz-checksum-crc64nvme`
** UploadPart: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, und `x-amz-checksum-crc64nvme`


* Unterstützung für  `x-amz-checksum-type` für CreateMultipartUpload und CompleteMultipartUpload hinzugefügt.
* Unterstützung für den  `s3:x-amz-copy-source`Richtlinienbedingungsschlüssel wurde hinzugefügt.
* Die S3-Objektsperre kann für bestehende Buckets aktiviert werden.
...
* Unterstützung für STS AssumeRole und Sitzungsrichtlinien wurde hinzugefügt. Siehe link:../s3/example-session-policies.html["ein Beispiel für eine Sitzungsrichtlinie"]. AssumeRole kann unter Mandantengruppen eingerichtet werden.
...
11,7
 a| 
* Hinzugefügt link:../s3/quick-reference-support-for-aws-apis.html["Schnelle Referenz: Unterstützte S3-API-Anforderungen"].
...
10,1
 a| 
Unterstützung für Multipart-Upload, Virtual-Hosted-Anfragen und v4-Authentifizierung wurde hinzugefügt.
...
|===
Weitere Informationen sind unter link:../s3/index.html#supported-versions["Unterstützte Versionen und Updates der S3 REST API"] verfügbar.
