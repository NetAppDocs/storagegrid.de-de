...
Admin-Nodes:: Bereitstellung von Verwaltungsdiensten wie Systemkonfiguration, Überwachung und Protokollierung. Bei der Anmeldung am Grid Manager wird eine Verbindung zu einem Admin Node hergestellt. Jedes Grid muss über einen primären Admin Node verfügen und kann zur Redundanz zusätzliche, nicht-primäre Admin Nodes haben. Eine Verbindung kann zu jedem Admin Node hergestellt werden, und jeder Admin Node zeigt eine ähnliche Ansicht des StorageGRID Systems an. Die meisten Wartungsverfahren sind jedoch mit dem primären Admin Node durchzuführen.
+
--
...
+
--
Während der Erstinstallation eines neuen Storage Node kann festgelegt werden, dass dieser nur für link:../primer/what-storage-node-is.html#types-of-storage-nodes["entweder Metadaten oder Daten speichern"] verwendet wird.
...
gdu-server
 a| 
Der Backend-Task-Runner für Grid-Wartungs- und Bereitstellungsaktionen. Akzeptiert Anfragen von  `mgmt-api` und anderen Hilfsskripten, validiert diese und leitet jede Aktion an den entsprechenden GDU-Task-Handler weiter, wie Bereitstellung, Erweiterung, Stilllegung und Aktualisierungen.
 a| 
Admin-Nodes
...
Hochverfügbarkeit
 a| 
Verwaltet hochverfügbare virtuelle IPs auf Knoten, die auf der Seite „High Availability Groups“ konfiguriert sind. Dieser Service ist auch als _keepalived_ Service bekannt.
 a| 
...
mgmt-api
 a| 
Die Backend-API des Grid Manager. Stellt authentifizierte REST-Endpunkte für die Administration des Grids (Konfiguration, Überwachung und Wartungsabläufe) bereit, setzt Validierungen und Berechtigungen durch und orchestriert lang andauernde Operationen. Die primäre Control-Plane-API, die von der Grid Manager-Oberfläche und Automatisierungsclients verwendet wird.
 a| 
Admin-Nodes
