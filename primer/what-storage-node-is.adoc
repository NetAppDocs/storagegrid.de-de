...
* Bei der Verwendung softwarebasierter Knoten ist sicherzustellen, dass die ausschließlich für Metadaten vorgesehenen Knotenressourcen mit den Ressourcen der vorhandenen Storage Nodes übereinstimmen. Beispielsweise:
+
...
Sie können einen kombinierten Storage Node in einen reinen Data-only Storage Node umwandeln. Wenn der Storage Node das <<adc-service,ADC-Dienst>> enthält und die Voraussetzungen für die Umwandlung erfüllt sind, wird der Service während der Umwandlung automatisch auf einen anderen Storage Node am selben Standort verschoben. Voraussetzungen und detaillierte Schritte sind unter link:../maintain/convert-to-data-only-node.html["Konvertieren eines Speicherknotens in einen Nur-Datenknoten"] zu finden.
...
Der ADC-Dienst verwaltet Topologieinformationen, einschließlich Standort und Verfügbarkeit von Diensten. Wenn ein Grid-Knoten Informationen von einem anderen Grid-Knoten benötigt oder eine Aktion von einem anderen Grid-Knoten ausgeführt werden soll, kontaktiert er einen ADC-Dienst, um den am besten geeigneten Grid-Knoten für die Bearbeitung seiner Anfrage zu finden. Zusätzlich behält der ADC-Dienst eine Kopie der Konfigurationspakete der StorageGRID Bereitstellung, sodass jeder Grid-Knoten aktuelle Konfigurationsinformationen abrufen kann.
