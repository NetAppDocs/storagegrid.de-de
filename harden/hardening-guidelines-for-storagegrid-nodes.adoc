...
* Um den Remote-IPMI-Zugriff auf den BMC im Grid Manager zu steuern, navigieren Sie zu *Konfiguration* > *Sicherheit* > *Sicherheitseinstellungen* > *Geräte* > *Remote-IPMI-Zugriff*:
+
...
== BMC-Passwort des Geräts ändern

Sie können das BMC Passwort für ein Gerät ändern, um den Zugriff einzuschränken und die Systemsicherheit zu verbessern.

. Im Grid Manager zu *Knoten* > *_grid node_* > *BMC settings* navigieren.
. Im Abschnitt „Passwort ändern“ im Feld *Neues Passwort* ein neues BMC Passwort mit einer Länge zwischen 8 und 16 Zeichen eingeben und anschließend im Feld *Neues Passwort bestätigen* erneut eingeben.
. Wählen Sie *Speichern*.


Das BMC Passwort kann auch über die  `PUT /bmc-configuration/root-password`API geändert werden.



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== [[control-remote-access-through-usb-ports]]Externer USB-Port-Zugriff auf Appliances steuern

Sie können die USB-Ports der Appliances aktivieren, um dem technischen Support Zugriff für Diagnose und Fehlerbehebung zu ermöglichen. Wenn kein Zugriff auf die USB-Ports erforderlich ist, sollten die Ports aus Sicherheitsgründen deaktiviert werden. USB-Ports sind nach der Erstinstallation oder einem Upgrade standardmäßig aktiviert.

* Im Grid Manager unter *Konfiguration* > *Sicherheit* > *Sicherheitseinstellungen* > *Geräte* > *USB-Anschlusszugriff* können die USB-Anschlüsse aktiviert oder deaktiviert werden:
+
** Das Kontrollkästchen *USB-Ports aktivieren* auswählen und speichern, um die Ports zu aktivieren.
+
Wenn das Kontrollkästchen „USB“ zuvor deaktiviert und gespeichert wurde, kann das Kontrollkästchen *USB-Anschlüsse aktivieren* ausgewählt und gespeichert werden, um die USB-Anschlüsse wieder zu aktivieren.

** Das Kontrollkästchen *USB-Anschlüsse aktivieren* wird deaktiviert und gespeichert, um die Anschlüsse zu deaktivieren.
...
* Unvertrauenswürdige Clients sollten keine direkte Verbindung zu Storage Nodes herstellen. Ein Load-Balancer-Endpunkt, der von einem Gateway Node oder einem Load Balancer eines Drittanbieters bereitgestellt wird, ist zu verwenden.
