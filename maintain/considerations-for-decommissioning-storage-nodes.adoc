...
---
= Überlegungen zur Stilllegung von Storage Node in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Das System muss jederzeit über ausreichend Speicherknoten verfügen, um die Betriebsanforderungen zu erfüllen, einschließlich der link:understanding-adc-service-quorum.html["ADC-Quorum"] und der aktiven link:reviewing-ilm-policy-and-storage-configuration.html["ILM-Richtlinie"]. Falls die Außerbetriebnahme eines Speicherknotens die Anzahl der ADC-Knoten an einem Standort unter das Quorum senken würde, verschiebt StorageGRID den ADC-Dienst automatisch auf verfügbare Nicht-ADC-Speicherknoten am selben Standort, wodurch das ADC-Quorum aufrechterhalten wird. Kann der ADC-Dienst nicht automatisch verschoben werden, kann es erforderlich sein, link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["einen neuen Storage Node in einer Erweiterungsoperation hinzufügen"] bevor ein vorhandener Speicherknoten außer Betrieb genommen werden kann.
+
