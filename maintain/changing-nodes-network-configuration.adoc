...
summary: Mit dem Tool „IP ändern“ kann die Netzwerkkonfiguration eines oder mehrerer Knoten geändert werden. Die Konfiguration des Grid Network kann geändert oder die Admin- oder Client-Netzwerke hinzugefügt, geändert oder entfernt werden. Auch die LAN-IP-Einstellungen für einen Appliance-Knoten BMC können geändert werden. 
---
= Netzwerkkonfiguration des Knotens in StorageGRID ändern
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Mit dem Tool „IP ändern“ kann die Netzwerkkonfiguration eines oder mehrerer Knoten geändert werden. Die Konfiguration des Grid Network kann geändert oder die Admin- oder Client-Netzwerke hinzugefügt, geändert oder entfernt werden. Auch die LAN-IP-Einstellungen für einen Appliance-Knoten BMC können geändert werden.
...
NOTE: Wenn Single Sign-On (SSO) für Ihr StorageGRID System aktiviert ist und Sie die IP-Adresse eines Admin Node ändern, ist zu beachten, dass jede relying party trust, die mit der IP-Adresse des Admin Node (anstatt mit seinem vollqualifizierten Domainnamen, wie empfohlen) konfiguriert wurde, ungültig wird. Eine Anmeldung an diesem Node ist dann nicht mehr möglich. Unmittelbar nach der Änderung der IP-Adresse muss die relying party trust des Node in Active Directory Federation Services (AD FS) mit der neuen IP-Adresse aktualisiert oder neu konfiguriert werden. Es wird auf die Anweisungen zur Einrichtung von SSO unter link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] oder link:../admin/learn-about-setting-up-sso-saml.html["SAML"] verwiesen.
...
+
image:change_ip_tool_main_menu.png["Screenshot mit Willkommensbildschirm des Change IP-Tools"]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png["Screenshot, der durch umgebenden Text beschrieben wird"]
...
+
image:change_ip_tool_validate_sample_error_messages.gif["Screenshot, der durch umgebenden Text beschrieben wird"]

+
In diesem Beispiel wurde die Validierung erfolgreich bestanden.

+
image:change_ip_tool_validate_sample_passed_messages.gif["Screenshot, der durch umgebenden Text beschrieben wird"]
...
== BMC LAN-IP-Einstellungen des Appliance-Knotens ändern

Die LAN-IP-Einstellungen für einen Appliance-Knoten BMC können geändert werden.

. Im Grid Manager zu *Knoten* > *_grid node_* > *BMC settings* navigieren.
. Im Abschnitt LAN-IP-Einstellungen einen IP-Zuweisungstyp von *Statisch* oder *DHCP* auswählen.
. Die MAC-Adresse, die IPv4-Adresse (CIDR) und das Standardgateway für den BMC eingeben.
. Wählen Sie *Speichern*.


Sie können die BMC LAN IP-Einstellungen auch über die  `PUT /bmc-configuration/root-password` oder  `GET /bmc-configuration/root-password` API ändern oder anzeigen.
