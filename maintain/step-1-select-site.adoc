...
---
= Wählen Sie einen Standort für die Außerbetriebnahme in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:decommission_site_step_select_site.png["Deaktivieren Sie Site Schritt 1 Select Site"]
...
| Möglichen Grund einer Deaktivierung | Beschreibung | Nächster Schritt 
 ...
 a| 
Grünes Häkchen (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"])
 a| 
...
Nein. Alle Knoten an diesem Standort sind getrennt. Wenden Sie sich an Ihren NetApp Account-Ansprechpartner.
 a| 
Eine Deaktivierung eines verbundenen Standorts ist nur möglich, wenn jeder Knoten im Standort verbunden ist (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"]).
 a| 
...
|===
+
Das Beispiel zeigt ein StorageGRID System mit drei Standorten. Das grüne Häkchen (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"]) bei den Standorten Raleigh und Sunnyvale zeigt an, dass diese Standorte außer Betrieb genommen werden können. Der Standort Vancouver kann jedoch nicht außer Betrieb genommen werden, da er den primären Admin Node enthält.
