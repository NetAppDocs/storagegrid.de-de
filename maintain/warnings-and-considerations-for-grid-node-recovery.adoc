...
---
= Warnungen und Hinweise zur Wiederherstellung von StorageGRID-Knoten
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Wenn ein anderer Grid-Knoten als der primäre Admin-Knoten wiederhergestellt wird, muss eine Verbindung zwischen dem wiederherzustellenden Grid-Knoten und dem Admin-Knoten bestehen, der das Wiederherstellungsverfahren verwaltet.
...
Fällt ein Server aus, der mehrere Grid-Knoten hostet, können die Knoten in beliebiger Reihenfolge wiederhergestellt werden. Wenn der ausgefallene Server jedoch den primären Admin Node hostet und keine nicht-primären Admin Nodes existieren, ist zuerst der primäre Admin Node wiederherzustellen.
