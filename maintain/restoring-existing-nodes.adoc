...
---
= StorageGRID-Knoten auf dem Host wiederherstellen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Wenn ein Knoten von einem nicht-primären Admin Node wiederhergestellt wird, ist ADMIN_IP auf die Grid Network-IP-Adresse des _non-primary Admin Node_ zu setzen, sodass sich der wiederhergestellte Knoten beim nicht-primären Admin Node und nicht beim primären Admin Node registriert. Die Regeln für Admin Network-Schlüssel sind unter link:../swnodes/creating-node-configuration-files.html["Erstellen Sie StorageGRID-Knotenkonfigurationsdateien für Linux-Bereitstellungen"] zu finden.
