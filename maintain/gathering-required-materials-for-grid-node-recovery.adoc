...
---
= Sammeln Sie die benötigten Materialien für die Wiederherstellung des StorageGRID-Grid-Knotens
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Wiederherstellungspaket `.zip` Datei
 a| 
Besorgen Sie sich eine Kopie des aktuellsten Wiederherstellungspakets `.zip` Datei:
`sgws-recovery-package-_id-revision_.zip`

Der Inhalt der `.zip` Die Datei wird bei jeder Änderung des Systems aktualisiert.  Sie werden angewiesen, nach der Durchführung solcher Änderungen die aktuellste Version des Wiederherstellungspakets an einem sicheren Ort zu speichern.  Verwenden Sie zur Wiederherstellung nach Netzausfällen die aktuellste Kopie.

Wenn Sie bei einem beliebigen Admin-Knoten angemeldet sind, verwenden Sie den Grid Manager, um ein Wiederherstellungspaket herunterzuladen.  Wählen Sie *Wartung* > *System* > *Wiederherstellungspaket*.
...
. Springen Sie zum entsprechenden Abschnitt für Ihre Plattform, um die benötigten Dateien basierend auf Ihrer Plattform und den Grid-Knoten auszuwählen, die Sie wiederherstellen müssen.
+
