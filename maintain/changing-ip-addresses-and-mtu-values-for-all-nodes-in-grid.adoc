...
---
= Ändern Sie die IP-Adressen aller Knoten in einem StorageGRID Grid
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Wenn SSO für Ihr StorageGRID System aktiviert ist und Vertrauensstellungen der vertrauenden Seite mithilfe von Admin-Node-IP-Adressen (anstatt vollqualifizierten Domainnamen, wie empfohlen) konfiguriert wurden, sollte unmittelbar nach einer Änderung der IP-Adressen eine Aktualisierung oder Neukonfiguration dieser Vertrauensstellungen in Active Directory Federation Services (AD FS) erfolgen. Siehe Einrichtung von SSO mit link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] oder link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
