...
---
= Wiederherstellung nach StorageGRID Storage Node-Ausfällen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Wenden Sie sich an den technischen Support.
 a| 
Wenn mehr als ein Storage Node ausgefallen ist, muss der technische Support bei der Wiederherstellung unterstützen, um Inkonsistenzen in der Cassandra-Datenbank zu verhindern, die zu Datenverlust führen könnten. Möglicherweise ist ein Site-Wiederherstellungsverfahren erforderlich.

Der technische Support kann feststellen, wann die Wiederherstellung eines zweiten Storage Node sicher beginnen kann.

*Hinweis*: Wenn an einem Standort mehr als ein Storage Node, der den ADC-Dienst enthält, ausfällt, gehen alle ausstehenden Plattformdienstanforderungen für diesen Standort verloren.
...
Ein oder mehrere Storage-Volumes sind ausgefallen, das Systemlaufwerk ist jedoch intakt
 a| 
link:reviewing-warnings-about-storage-volume-recovery.html["Wiederherstellung nach einem Storage-Volume-Ausfall bei intaktem Systemlaufwerk"]
 a| 
