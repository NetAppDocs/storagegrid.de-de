...
summary: 'Möglicherweise müssen Sie einen Knoten außer Betrieb setzen, der derzeit nicht mit dem Grid verbunden ist (einen Node, dessen Status unbekannt oder administrativ ausgefallen ist).' 
---
...
.Über diese Aufgabe
Nicht verbundene Knoten lassen sich anhand des blauen Symbols „Unbekannt“ image:icon_alarm_blue_unknown.png["Symbol Unbekannt"] oder des grauen Symbols „Administrativ deaktiviert“ image:icon_alarm_gray_administratively_down.png["Symbol administrativ nicht verfügbar"] in der Spalte *Status* erkennen.
...
. Die Liste der Knoten kann überprüft und *OK* ausgewählt werden.
+
...
. Sobald das neue Wiederherstellungspaket verfügbar ist, kann über den Link oder über *Wartung* > *System* > *Wiederherstellungspaket* auf die Seite mit dem Wiederherstellungspaket zugegriffen werden. Anschließend die  `.zip` Datei herunterladen.
+
...
--
Zur Überwachung der Reparatur von erasure-codierten Daten und zum erneuten Versuch eventuell fehlgeschlagener Anfragen kann unter *Support* > *Tools* > *Metrics* die geschätzte Zeit bis zum Abschluss sowie der Fertigstellungsprozentsatz des aktuellen Auftrags eingesehen werden. Anschließend steht im Grafana-Bereich *EC Overview* zur Verfügung. Die Dashboards *Grid EC Job Estimated Time to Completion* und *Grid EC Job Percentage Completed* liefern die entsprechenden Informationen.
