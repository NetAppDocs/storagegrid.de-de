...
---
= Alle StorageGRID-Knoten aus- und wieder einschalten
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Wenn alle Knoten an einem Standort oder im Grid ausgeschaltet sind, ist kein Zugriff auf die aufgenommenen Objekte möglich, solange die Storage Nodes offline sind.
...
.Über diese Aufgabe
Zuerst die Dienste auf den Admin-Knoten und Gateway-Knoten beenden, anschließend die Dienste auf den Storage Nodes stoppen.
...
NOTE: Wenn ein Host mehrere Grid-Knoten umfasst, gehen die Knoten beim Einschalten des Hosts automatisch wieder online.

.Schritte
. Schalten Sie die Hosts für den primären Admin-Node und alle nicht-primären Admin-Nodes ein.
+

NOTE: Sie können sich erst dann bei den Admin-Knoten anmelden, wenn die Storage Nodes neu gestartet wurden.
