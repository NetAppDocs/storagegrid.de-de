...
summary: Die Admin-Node-Datenbank bei der Wiederherstellung eines primären Admin-Node mithilfe eines nicht-primären Admin-Node wiederherstellen. 
---
= Die StorageGRID Admin Node Datenbank wird beim Wiederherstellen des primären Admin Node wiederhergestellt.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.. Fügen Sie den privaten SSH-Schlüssel zum SSH-Agenten hinzu. Geben Sie `ssh-add` ein.
...
.. Wenn kein passwortloser Zugriff auf andere Server mehr erforderlich ist, den privaten Schlüssel aus dem SSH-Agenten entfernen. `ssh-add -D` eingeben.
