...
summary: 'Sie können NTP-Server (Network Time Protocol) hinzufügen, aktualisieren oder entfernen, um sicherzustellen, dass die Daten zwischen den Grid-Nodes im StorageGRID-System genau synchronisiert werden.' 
---
= NTP-Server in StorageGRID verwalten
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Bei der Angabe der externen NTP-Quelle für eine StorageGRID Produktionsinstallation darf der Windows-Zeitdienst (W32Time) unter Windows-Versionen vor Windows Server 2016 nicht verwendet werden. Der Zeitdienst älterer Windows-Versionen ist nicht ausreichend genau und wird von Microsoft für den Einsatz in Umgebungen mit hohen Genauigkeitsanforderungen, einschließlich StorageGRID, nicht unterstützt. Weitere Einzelheiten sind unter  https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["Support-Grenze für hohe Genauigkeit der Zeit"^] zu finden.
