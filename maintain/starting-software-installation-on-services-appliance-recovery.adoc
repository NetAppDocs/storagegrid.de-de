...
---
= Starten Sie die Installation der StorageGRID-Software auf einem Services-Gerät
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Wenn Sie einen Gateway-Knoten oder einen nicht-primären Admin-Knoten installieren, kennen Sie die IP-Adresse für einen der folgenden Admin-Knoten:
+
** Der primäre Admin-Node für das StorageGRID Grid
** Ein nicht-primärer Admin-Knoten, falls einer verfügbar ist und zur Wiederherstellung des Knotens verwendet wird


* Wenn Sie einen Knoten (einschließlich des Primary Admin Node) mithilfe eines nicht primären Admin Node wiederherstellen, kann die IP-Adresse dieses Admin Node angegeben werden.
...
* Für einen primären Admin-Knoten geben Sie den Namen des Knotens an und laden anschließend die erforderlichen Softwarepakete hoch (falls erforderlich). Wenn Sie den primären Admin-Knoten wiederherstellen und in Ihrem Grid ein nicht-primärer Admin-Knoten vorhanden ist, besteht die Möglichkeit, den Knoten entweder mithilfe eines nicht-primären Admin-Knotens wiederherzustellen und die IP-Adresse dieses Admin-Knotens anzugeben, oder den Knoten mit Ihrer aktuellen Konfiguration zu installieren bzw. wiederherzustellen.
...
.. Im Feld *Knotenname* ist derselbe Name einzugeben, der für den wiederherzustellenden Knoten verwendet wurde, und *Speichern* ist auszuwählen.
.. Wählen Sie im Abschnitt „Installationsmodus“ den passenden Modus aus den beiden verfügbaren Optionen. Es kann entweder die Methode „Knoten installieren oder diesen Knoten mit der aktuellen Konfiguration wiederherstellen“ oder die Methode „Diesen Knoten mithilfe eines nicht-primären Admin Node wiederherstellen“ ausgewählt werden.
+
[role="tabbed-block"]
====
.Aktuelle Konfigurationsmethode
--
Einen Knoten installieren oder diesen Knoten mit der aktuellen Konfiguration wiederherstellen.

... Im Abschnitt „Installation“ ist die unter „Aktueller Status“ aufgeführte Softwareversion zu überprüfen. Ist die zur Installation bereitstehende Softwareversion korrekt, kann zum <<installation_section_step,Installationsschritt>> übergegangen werden.
... Falls Sie eine andere Softwareversion hochladen müssen, wählen Sie im Menü *Erweitert* die Option *StorageGRID software hochladen*. Die Seite „StorageGRID software hochladen“ wird angezeigt.
... Wählen Sie *Durchsuchen*, um das *Softwarepaket* und die *Prüfsummendatei* für StorageGRID Software hochzuladen. Die Dateien werden nach der Auswahl automatisch hochgeladen.
... *Startseite* auswählen, um zur StorageGRID Appliance Installer Startseite zurückzukehren.


--
.Methode für nicht-primären Admin-Node
--
Dieser Knoten kann mithilfe eines nicht-primären Admin-Knotens wiederhergestellt werden.

... Geben Sie im Feld „IP des nicht-primären Admin-Knotens“ die Grid-Netzwerk-IP-Adresse des nicht-primären Admin-Knotens ein, den Sie für die Wiederherstellung verwenden möchten, und wählen Sie *Speichern*.
...
+
Wenn die Schaltfläche *Installation starten* nicht aktiviert ist, müssen Sie möglicherweise die Netzwerkkonfiguration oder die Porteinstellungen ändern.  Beachten Sie die Wartungsanleitung Ihres Geräts.

. Wählen Sie auf der Startseite des StorageGRID Appliance Installers die Option *Installation starten*.
+
Der aktuelle Status ändert sich zu „Installation läuft“, und die Monitor installation Seite wird angezeigt.

+

NOTE: Falls Sie die Seite zur Monitorinstallation manuell aufrufen müssen, wählen Sie in der Menüleiste *Monitor installation* aus.
