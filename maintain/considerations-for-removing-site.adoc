...
---
= Überlegungen zur Standortstilllegung in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Wenn eine Site eine Mischung aus verbundenen (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"]) und getrennten Knoten (image:icon_alarm_gray_administratively_down.png["Icon Alarm Grau Administrativ Nach Unten"] oder image:icon_alarm_blue_unknown.png["Symbolalarm Blau Unbekannt"]) enthält, müssen alle Offline-Knoten wieder online geschaltet werden.
...
* Sie können einen Standort nicht außer Betrieb nehmen, wenn er eine Mischung aus verbundenen (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"]) und getrennten (image:icon_alarm_blue_unknown.png["Symbolalarm Blau Unbekannt"] oder image:icon_alarm_gray_administratively_down.png["Icon Alarm Grau Administrativ Nach Unten"]) Knoten enthält.
* Ein Standort kann nicht außer Betrieb genommen werden, wenn ein Knoten an einem anderen Standort getrennt ist (image:icon_alarm_blue_unknown.png["Symbolalarm Blau Unbekannt"] oder image:icon_alarm_gray_administratively_down.png["Icon Alarm Grau Administrativ Nach Unten"]).
...
* Alle Knoten in Ihrem StorageGRID System müssen den Verbindungsstatus *Verbunden* (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"]) aufweisen; allerdings können auf den Knoten aktive Warnmeldungen vorhanden sein.
+
...
+
** *Unbekannt* (image:icon_alarm_blue_unknown.png["Symbolalarm Blau Unbekannt"]): Aus unbekanntem Grund ist ein Knoten getrennt oder Dienste auf dem Knoten sind unerwartet ausgefallen. Beispielsweise könnte ein Dienst auf dem Knoten beendet worden sein oder der Knoten könnte aufgrund eines Stromausfalls oder einer unerwarteten Unterbrechung seine Netzwerkverbindung verloren haben.
** *Administrativ außer Betrieb* (image:icon_alarm_gray_administratively_down.png["Icon Alarm Grau Administrativ Nach Unten"]): Der Knoten ist aus einem erwarteten Grund nicht mit dem Grid verbunden. Beispielsweise wurden der Knoten oder die Dienste auf dem Knoten ordnungsgemäß heruntergefahren.


* Alle Knoten an allen anderen Standorten müssen den Verbindungsstatus *Verbunden* (image:icon_alert_green_checkmark.png["Symbol Warnung Grün Häkchen"]) aufweisen; allerdings können diese anderen Knoten aktive Warnmeldungen haben.
