...
---
= Seite „Knoten außer Betrieb nehmen“ in StorageGRID aufrufen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
In diesem Beispiel zeigt ein grünes Häkchen in der Spalte „Decommission Possible“ an, dass der API Gateway Node und der data-only Storage Node deaktiviert werden können. Die anderen Nodes können aus verschiedenen Gründen nicht deaktiviert werden. Beispielsweise kann der primäre Admin Node nicht deaktiviert werden, da die Deaktivierung dieses Node-Typs nicht unterstützt wird. Der metadata-only Storage Node kann nicht deaktiviert werden, da der Standort dann die link:../maintain/understanding-adc-service-quorum.html["ADC-Quorum"] nicht mehr erfüllen würde.

+
image:decommission_nodes_page.png["Die Seite Knoten ausmustern"]
...
** image:icon_alarm_gray_administratively_down.png["Graues Fragensymbol"] (grau): Administrativ deaktiviert
** image:icon_alarm_blue_unknown.png["Blaues Fragezeichen-Symbol"] (blau): Unbekannt
...
Bearbeiten Sie die HA-Gruppe, um die Schnittstelle des Node zu entfernen, oder entfernen Sie die gesamte HA-Gruppe. Siehe link:../admin/configure-high-availability-group.html["Konfigurieren Sie Hochverfügbarkeitsgruppen"].
 ...
 a| 
Nein, Standort _x_ benötigt mindestens 3 Storage Nodes mit ADC-Diensten oder verfügbare Nicht-ADC-Knoten, auf die die ADC-Dienste verlagert werden können. Die Außerbetriebnahme von ADC-Knoten ist nicht möglich, wenn Knoten offline sind.
 a| 
*Nur Speicherknoten* (kombinierte oder reine Metadaten-Speicherknoten). Ein Speicherknoten kann nicht außer Betrieb genommen werden, wenn am Standort nicht genügend Knoten verbleiben, um die Quorum-Anforderungen des ADC zu erfüllen und der ADC-Dienst nicht automatisch verschoben werden kann. Für die automatische Verschiebung müssen mehr als zwei ADC-Speicherknoten im gesamten Grid online sein und am selben Standort Nicht-ADC-Speicherknoten verfügbar sein.
 a| 
Es muss sichergestellt sein, dass die Bedingungen für die automatische ADC-Dienstverschiebung erfüllt sind (siehe link:../maintain/understanding-adc-service-quorum.html["ADC-Quorum"]), oder link:../maintain/move-adc-service.html["Den ADC Service manuell verschieben"] auf einen anderen Storage Node am selben Standort verschieben. Eine Erweiterung ist auch möglich, indem dem Standort ein neuer Storage Node hinzugefügt und festgelegt wird, dass dieser einen ADC-Dienst haben soll. Siehe link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["Workflow-Zusammenfassung für das Hinzufügen von Grid-Knoten zu StorageGRID"].
