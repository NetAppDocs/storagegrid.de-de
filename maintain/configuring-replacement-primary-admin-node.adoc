...
---
= Konfigurieren Sie einen Ersatz-Primären Admin-Node in StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. *Ausgefallenen primären Admin-Knoten wiederherstellen* auswählen.
+
image:install_or_recover_primary_admin_node.png["Installieren oder Wiederherstellen des primären Admin-Knotens"]

. Laden Sie die aktuellste Sicherung des Wiederherstellungspakets hoch:
+
.. Wählen Sie *Durchsuchen*.
.. Die aktuellste Wiederherstellungspaketdatei für Ihr StorageGRID System suchen und *Öffnen* auswählen.
...
NOTE: Um eine Vertrauensstellung für eine vertrauende Partei zu konfigurieren, siehe Einrichtung von SSO mit link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] oder link:../admin/learn-about-setting-up-sso-saml.html["SAML"]. Für den Zugriff auf das Standardserverzertifikat ist eine Anmeldung an der Befehlsshell des Admin Node erforderlich. Zum  `/var/local/mgmt-api` Verzeichnis wechseln und die  `server.crt` Datei auswählen.
