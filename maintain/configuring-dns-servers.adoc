...
summary: 'Sie können DNS-Server hinzufügen, entfernen und aktualisieren, sodass Sie vollqualifizierte Domainnamen (FQDN) anstelle von IP-Adressen verwenden können.' 
---
= DNS-Server in StorageGRID konfigurieren
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Schritte
. Wählen Sie *Wartung* > *Netzwerk* > *DNS-Server*.
. Das Löschsymbol image:icon-x-to-remove.png["Symbol Löschen"] neben der IP-Adresse auswählen.
. Wählen Sie *Speichern*.
