...
* Sie kennen die IP-Adresse eines der folgenden Admin Nodes:
+
** Der primäre Admin-Node für das StorageGRID Grid
** Ein nicht-primärer Admin Node, falls verfügbar
...
* Sie geben die IP-Adresse des primären Admin-Knotens an. Wenn der primäre Admin-Knoten nicht verfügbar ist, geben Sie die IP-Adresse des nicht-primären Admin-Knotens an, sofern verfügbar. Sie bestätigen außerdem den Hostnamen (Systemnamen) des Knotens.
...
. Im Feld *Knotenname* ist derselbe Hostname (Systemname) einzugeben, der für den wiederherzustellenden Knoten verwendet wurde, und *Speichern* auszuwählen.
. Im Abschnitt „Installation“ sollte der aktuelle Status „Bereit zum Starten der Installation von  `_node name_` in das Grid mit Primary Admin Node `_admin_ip_`“ angezeigt werden und die Schaltfläche *Start installation* aktiviert sein.
+
...
NOTE: Falls Sie die Monitor-Installationsseite manuell aufrufen müssen, wählen Sie in der Menüleiste *Monitor installation* aus. Siehe  https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/monitoring-appliance-installation.html["Überwachen Sie die Appliance-Installation"^].
