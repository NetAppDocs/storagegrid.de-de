...
---
= Führen Sie eine StorageGRID Erweiterung durch
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Schritte
. Wählen Sie *Wartung* > *Aufgaben* > *Grid-Erweiterung*.
+
Die Seite zur Grid-Erweiterung wird angezeigt. Im Abschnitt „Ausstehende Knoten“ sind die Knoten aufgeführt, die zum Hinzufügen bereit sind.
...
+
image:grid_node_details.png["Screenshot mit Grid Node-Details"]
...
ADC-Service (kombinierte oder nur Metadaten-Storage-Nodes)
 a| 
Ob dieser Speicherknoten den Administrative Domain Controller (ADC)-Dienst ausführen wird.  Der ADC-Dienst verfolgt den Standort und die Verfügbarkeit von Grid-Diensten.  Mindestens drei Speicherknoten an jedem Standort müssen den ADC-Dienst enthalten.

*** Wählen Sie *Ja*, wenn der Speicherknoten, den Sie ersetzen, den ADC-Dienst enthält.  Da Sie einen Speicherknoten nicht außer Betrieb nehmen können, wenn zu wenige ADC-Dienste übrig bleiben, stellt diese Einstellung sicher, dass ein neuer ADC-Dienst verfügbar ist, bevor der alte Dienst entfernt wird.
*** Du kannstlink:../maintain/move-adc-service.html["Verschieben Sie den ADC-Dienst auf einen anderen Speicherknoten am selben Standort"] um sicherzustellen, dass das ADC-Service-Quorum erreicht wird.
...
. Wenn alle Grid-Knoten genehmigt wurden, die *Bereitstellungs-Passphrase* eingeben und *Erweitern* auswählen.
+
