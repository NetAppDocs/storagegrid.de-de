...
summary: Nach dem Hinzufügen von Storage Nodes kann das Erasure-Coding-Rebalance-Verfahren verwendet werden, um erasure-codierte Fragmente zwischen den vorhandenen und neuen Storage Nodes neu zu verteilen. 
---
= Nach dem Hinzufügen von Knoten zu Ihrem StorageGRID -System sollten Sie die mit Erasure-Codierung versehenen Daten neu ausbalancieren.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** Die Daten in allen Knoten liegen innerhalb einer Abweichung von 5 % von der angestrebten nutzbaren Speicherplatzpartition.
...
+
Der EC Rebalance-Vorgang gilt als abgeschlossen und wird beendet, sobald der nutzbare Speicherplatz auf allen Knoten innerhalb einer Abweichung von 5 % von der Zielaufteilung des nutzbaren Speicherplatzes liegt.
...
+
Der nutzbare Speicherplatz sollte nun gleichmäßiger auf die Storage Nodes am Standort verteilt sein.
